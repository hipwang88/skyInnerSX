...
@implementation skyDVISignalView

@synthesize isOpen = _isOpen;
@synthesize selectIndex = _selectIndex;
@synthesize dviInputs = _dviInputs;

#pragma mark - skyDVISignalView Basic methods
...
#pragma mark - skyDVISignalView Private methods
// 数据初始化
- (void)initializeDefaults
{
...
- (void)didSelectCellRowFirstDo:(BOOL)firstDoInsert nextDo:(BOOL)nextDoInsert
{
    ...
    int contentCount;
    if (_selectIndex.section != _dviInputs / GROUP_NUMBER)
        contentCount = GROUP_NUMBER;
    else
        contentCount = _dviInputs % GROUP_NUMBER;
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    if (_dviInputs <= GROUP_NUMBER)
    {
        nResult = 1;
    }
    else
    {
        nResult = (_dviInputs-1) / GROUP_NUMBER + 1;
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    else
    {
        if (self.isOpen)
        {
            ...
            {
                if (section != _dviInputs / GROUP_NUMBER)
                    return GROUP_NUMBER + 1;
                else
                    return _dviInputs % GROUP_NUMBER + 1;
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_dviInputs <= GROUP_NUMBER)
    {
        ...
        cell.titleLabel.text = [NSString stringWithFormat:@"DVI-%d",nIndex];
        cell.imageShow.image = [UIImage imageNamed:@"signal_DVI-Small.png"];
        // 别名
        cell.titleAlias.text = [_myDataSource getDVIMatrixAliasAtIndex:(int)indexPath.row+1];
...
{
    if (_dviInputs <= GROUP_NUMBER)  // 小于等于16不进行分组
    {
        // 切换信号
        [_myDelegate haveSignal:SIGNAL_DVI SwitchTo:(int)indexPath.row+1];
    }
    else                    // 分组需要展开
    {
        if (indexPath.row == 0)
