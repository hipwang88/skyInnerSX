...
@property (assign, nonatomic) int cvbsInputs;
...
@implementation skyCVBSSignalView

@synthesize isOpen = _isOpen;
@synthesize selectIndex = _selectIndex;
@synthesize cvbsInputs = _cvbsInputs;

#pragma mark - skyCVBSSignalView Basic methods
...
- (void)viewDidLoad
{
    ...
    //self.title = @"CVBS信号切换";
    self.title = [NSString stringWithFormat:@"%@ CVBS",NSLocalizedString(@"WinMenu_SwitchSignal", nil)];
    _isOpen = NO;
}
...
#pragma mark - skyCVBSSignalView Private methods
// 数据初始化
- (void)initializeDefaults
{
    _cvbsInputs = [_myDataSource getCVBSMatrixInputs];
...
- (void)didSelectCellRowFirstDo:(BOOL)firstDoInsert nextDo:(BOOL)nextDoInsert
{
    ...
    int contentCount;
    if (_selectIndex.section != _cvbsInputs / GROUP_NUMBER)
        contentCount = GROUP_NUMBER;
    else
        contentCount = _cvbsInputs % GROUP_NUMBER;
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    if (_cvbsInputs <= GROUP_NUMBER)
    {
        nResult = 1;
    }
    else
    {
        nResult = (_cvbsInputs-1) / GROUP_NUMBER + 1;
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    else
    {
        if (self.isOpen)
        {
            ...
            {
                if (section != _cvbsInputs / GROUP_NUMBER)
                    return GROUP_NUMBER + 1;
                else
                    return _cvbsInputs % GROUP_NUMBER + 1;
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_cvbsInputs <= GROUP_NUMBER)
    {
        ...
        cell.titleLabel.text = [NSString stringWithFormat:@"CVBS-%d",nIndex];
        cell.imageShow.image = [UIImage imageNamed:@"signal_CVBS-Small.png"];
        // 别名
        cell.titleAlias.text = [_myDataSource getCVBSMatrixAliasAtIndex:(int)indexPath.row+1];
    ...
    else
    {
        ...
        else
        {
            ...
            if (indexPath.section == _cvbsInputs / GROUP_NUMBER)
                nEnd = _cvbsInputs;
    
            cell.titleLabel.text = [NSString stringWithFormat:@"CVBS  %d ~ %d",nBegin,nEnd];
...
{
    if (_cvbsInputs <= GROUP_NUMBER)  // 小于等于16不进行分组
    {
        // 切换信号
        [_myDelegate haveSignal:SIGNAL_CVBS SwitchTo:(int)indexPath.row+1];
    }
    else                    // 分组需要展开
    {
        if (indexPath.row == 0)
        ...
        else
        {
            ...
            // 代理调用进行切换
            [_myDelegate haveSignal:SIGNAL_CVBS SwitchTo:nPath];
        }
    }
