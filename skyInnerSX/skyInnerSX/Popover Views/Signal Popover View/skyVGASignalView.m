...
@implementation skyVGASignalView

@synthesize isOpen = _isOpen;
@synthesize selectIndex = _selectIndex;
@synthesize vgaInputs = _vgaInputs;

#pragma mark - skyVGASignalView Basic methods
...
#pragma mark - skyVGASignalView Private methods
// 数据初始化
- (void)initializeDefaults
{
...
- (void)didSelectCellRowFirstDo:(BOOL)firstDoInsert nextDo:(BOOL)nextDoInsert
{
    ...
    int contentCount;
    if (_selectIndex.section != _vgaInputs / GROUP_NUMBER)
        contentCount = GROUP_NUMBER;
    else
        contentCount = _vgaInputs % GROUP_NUMBER;
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    if (_vgaInputs <= GROUP_NUMBER)
    {
        nResult = 1;
    }
    else
    {
        nResult = (_vgaInputs-1) / GROUP_NUMBER + 1;
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    else
    {
        if (self.isOpen)
        {
            ...
            {
                if (section != _vgaInputs / GROUP_NUMBER)
                    return GROUP_NUMBER + 1;
                else
                    return _vgaInputs % GROUP_NUMBER + 1;
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_vgaInputs <= GROUP_NUMBER)
    {
        ...
        cell.titleLabel.text = [NSString stringWithFormat:@"VGA-%d",nIndex];
        cell.imageShow.image = [UIImage imageNamed:@"signal_VGA-Small.png"];
        // 别名
        cell.titleAlias.text = [_myDataSource getVGAMatrixAliasAtIndex:(int)indexPath.row+1];
...
{
    if (_vgaInputs <= GROUP_NUMBER)  // 小于等于16不进行分组
    {
        // 切换信号
        [_myDelegate haveSignal:SIGNAL_VGA SwitchTo:(int)indexPath.row+1];
    }
    else                    // 分组需要展开
    {
        if (indexPath.row == 0)
